...
- Zenn
  - 半角英数字（a-z0-9）、ハイフン（-）、アンダースコア（_）の12〜50文字の組み合わせにする必要があります。12文字を満たせない場合は、`yyyy-mm-dd-`を前に着けるなどしてください。

- Qiita
  - 初回時の制約はないが、基本的にはZennに合わせる。mainにpushすると、Github Action内のQiita CLIにより自動で命名が`<UUID>.md`に変更されます。
...
6. ユーザーレビュー
  - まずは「タイトル案」を何通りか提案する。ユーザーから合意が取れれば確定する
  - 次に内容のレビュー依頼をする。必要に応じて修正し、完成するまで繰り返す

7. 完成の合意が取れたら最後にもう一度LINT系を全実行し、問題なければCommit,Push（mainブランチでOK）
...
- 全体の「ストーリー」を意識。各章が独立するのではなく、全体としてまとまりのあるように
- 画像があった方が良ければプレースホルダとしてPATHを記載しておく。ユーザーが撮影し`images/`配下に格納します。
  - 画像の命名規則：`/images/記事ファイル名/01.png` (`01`部分は記事が出てくる順に採番)(例: `/images/zenn-qiita-crosspost/01.png`)
