...
![ZennのGitHub連携設定画面](https://cdn.jsdelivr.net/gh/endo-ava/article@main/images/zenn-qiita-crosspost/01.png)
...
![ZennのGitHub連携設定画面](https://cdn.jsdelivr.net/gh/endo-ava/article@main/images/zenn-qiita-crosspost/02.png)

「Only select repositories」-> 連携したいリポジトリ を選択し、「Install & Authrize」。

![ZennのGitHub連携設定画面](https://cdn.jsdelivr.net/gh/endo-ava/article@main/images/zenn-qiita-crosspost/03.png)
...
![Qiitaのトークン発行画面](https://cdn.jsdelivr.net/gh/endo-ava/article@main/images/zenn-qiita-crosspost/05.png)

必要な権限（read_qiita、write_qiita）を付与して発行します。アクセストークンの説明は自分が分かれば何でもよいです。

発行したトークンは再表示できないため大切に保管します。

![Qiitaのトークン発行画面](https://cdn.jsdelivr.net/gh/endo-ava/article@main/images/zenn-qiita-crosspost/06.png)

### 3.2. GitHub Secretsにトークンを登録

連携したいリポジトリのSettings → Secrets and variables → Actions から、`QIITA_TOKEN` という名前で、前述で発行したトークンを登録します。

![GitHub Secrets設定画面](https://cdn.jsdelivr.net/gh/endo-ava/article@main/images/zenn-qiita-crosspost/07.png)
...
![Zennに投稿された記事](https://cdn.jsdelivr.net/gh/endo-ava/article@main/images/zenn-qiita-crosspost/08.png)

Qiitaでテスト記事投稿成功🎉

![Qiitaに投稿された記事](https://cdn.jsdelivr.net/gh/endo-ava/article@main/images/zenn-qiita-crosspost/09.png)

補足として、Zenn側でデプロイが成功すると、通知が来ます。

![Zennのデプロイ成功通知](https://cdn.jsdelivr.net/gh/endo-ava/article@main/images/zenn-qiita-crosspost/10.png)

また、Qiita側では、CIによりQiitaCLIが以下のようなコミットを発行してきます。

主にFront Matterの自動設定コミットで、UUIDなどが自動で付与されます。

![投稿後のYAML差分](https://cdn.jsdelivr.net/gh/endo-ava/article@main/images/zenn-qiita-crosspost/11.png)
...
画像リンクは、ZennとQiitaで扱いが異なるため注意が必要です。

- **Zenn**: `../images/...` のような相対パスは正しく解決されず、画像が表示されませんでした。`/images/...` を使えばうまくいきました。

- **Qiita**: GitHub連携（Qiita CLI経由）では、ローカル画像の自動アップロードはされません。本文内の画像は公開URLで参照する必要があるため、今回は `https://cdn.jsdelivr.net/gh/...`（jsDelivr）を利用しています。

例:

```text
![画像1](https://cdn.jsdelivr.net/gh/<githubアカウント名>/<リポジトリ名>@<ブランチ名>/<対象画像への相対PATH>)
```
